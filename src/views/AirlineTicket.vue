...
const { airtickets, loading, importLoading } = storeToRefs(airticketStore);
...
const importActionHandler = async () => {
  ...
  try {
    ...
    const res = await airticketStore.importAction(frmData);
    fileImport.value = null;
    console.log(res);
    toast.success(`air line tickets ${res.message}`);
  } catch (e) {
