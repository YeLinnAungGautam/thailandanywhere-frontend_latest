...
const groupedReceipts = computed(() => {
  ...
  receipts.value.data.forEach((item) => {
    // Extract date part only (assuming date is in format YYYY-MM-DD)
    ...
    // Create a copy with time separated
...
const getTransactionType = (item) => {
  if (item.table_source === "booking_receipt") return "Booking Payment";
  if (item.interact_bank === "personal") return "Personal Transfer";
        ...
        <div class="grid grid-cols-3 gap-4">
          <div class="overflow-x-auto col-span-3">
            ...
            <div
              ...
              class="mb-6"
            >
              <div class="bg-gray-100 p-2 rounded-t-lg">
            ...
            <div v-if="loading">
              <div class="text-center py-10 text-xs">Loading...</div>
            </div>

            <div
              v-if="!loading && (!receipts?.data || receipts.data.length === 0)"
          ...
          >
            ...
            >
              ...
              <div>
                <img :src="placeholderFile" alt="" class="w-full" />
              </div>
            </DialogPanel>
          </Modal>
