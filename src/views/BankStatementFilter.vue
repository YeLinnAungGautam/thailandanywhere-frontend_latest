  ...
  <Layout :is_white="true" class="relative">
    ...
    <div class="">
      ...
      <div class="pb-4 space-y-3">
        ...
        <div
          class="flex justify-between items-center mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200"
        >
          <div class="flex space-x-4 items-center">
            ...
            <select
              v-model="sortBy"
              class="px-3 py-2 text-xs border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF613c]/20 bg-white"
            >
              <option
            ...
            <label class="text-xs font-semibold text-gray-700">Order:</label>
            <select
              v-model="sortOrder"
              class="px-3 py-2 text-xs border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF613c]/20 bg-white"
            >
            ...
            <button
              ...
              title="Reset to default sorting (Newest groups first)"
            >
              Reset
            </button>
          </div>

          <!-- Sort indicator and info -->
          <div class="flex items-center space-x-2">
            <div
              class="flex items-center px-3 py-1 bg-white border border-gray-200 rounded-lg text-xs"
            >
              <span class="text-gray-600 mr-2">
                {{ sortBy === "datetime" ? "📅" : "👤" }}
              </span>
              <span class="font-medium">{{ getSortingDescription }}</span>
              <span class="ml-2">
                {{ sortOrder === "desc" ? "⬇️" : "⬆️" }}
              </span>
            </div>
            ...
            >
              {{ Object.keys(groupedReceipts).length }} date groups
            </div>
          </div>
        </div>
      </div>
...
// Add these reactive variables to your script setup
const sortBy = ref("datetime"); // 'datetime' or 'name'
const sortOrder = ref("desc"); // 'asc' or 'desc'

// Add sorting options
const sortOptions = [
  { value: "datetime", label: "Date Groups" },
  { value: "name", label: "Receiver Name" },
];
...
// SIMPLE: Updated groupedReceipts with straightforward alphabetical sorting
const groupedReceipts = computed(() => {
  if (!cashImages.value?.data) return {};

  // Filter out App\\Models\\Booking relatable_type
  const filteredData = cashImages.value.data.filter(
    (item) => item.relatable_type !== "App\\Models\\Booking"
  );
  ...
  // SIMPLE SORTING: Sort items within each date group
  Object.keys(groups).forEach((date) => {
    groups[date].sort((a, b) => {
      if (sortBy.value === "name") {
        // Simple receiver name sorting
        const nameA = (a.receiver || "").toLowerCase();
        const nameB = (b.receiver || "").toLowerCase();
        ...
        // For datetime, keep original order or sort by time
        return 0; // Keep original order within the group
      }
    });
  });

  // Sort the groups by date if datetime is selected
  if (sortBy.value === "datetime") {
    const sortedGroups = Object.entries(groups).sort(([dateA], [dateB]) => {
      ...
      if (sortOrder.value === "desc") {
        ...
        return dateObjA.getTime() - dateObjB.getTime(); // Oldest first
      }
    });
    ...
    sortedGroups.forEach(([date, items]) => {
      orderedGroups[date] = items;
    });
    return orderedGroups;
  }

  // For name sorting, also sort the groups by first receiver name
  if (sortBy.value === "name") {
    const sortedGroups = Object.entries(groups).sort(
      ([, itemsA], [, itemsB]) => {
        const firstNameA = (itemsA[0]?.receiver).toLowerCase();
        const firstNameB = (itemsB[0]?.receiver).toLowerCase();
...
// Alternative: Compact sorting control options
const sortingOptions = [
  {
    value: "datetime-desc",
    label: "📅 Date Groups: Newest First",
    sortBy: "datetime",
    order: "desc",
  },
  {
    value: "datetime-asc",
    label: "📅 Date Groups: Oldest First",
    sortBy: "datetime",
    order: "asc",
  },
  {
    value: "name-asc",
    label: "👤 Groups by Name: A-Z",
    sortBy: "name",
    order: "asc",
  },
  {
    value: "name-desc",
    label: "👤 Groups by Name: Z-A",
    sortBy: "name",
    order: "desc",
  },
];

const selectedSorting = ref("datetime-desc");

// Watch for sorting changes and update individual sort variables
watch(selectedSorting, (newValue) => {
  const option = sortingOptions.find((opt) => opt.value === newValue);
  if (option) {
...
// Helper function to get sorting description for display
const getSortingDescription = computed(() => {
  if (sortBy.value === "datetime") {
    const orderText =
      sortOrder.value === "desc" ? "Newest First" : "Oldest First";
    return `Date Groups (${orderText})`;
  } else {
    const orderText = sortOrder.value === "desc" ? "Z-A" : "A-Z";
    return `Groups by Name (${orderText})`;
  }
});
...
const resetSorting = () => {
  ...
  selectedSorting.value = "datetime-desc";
};
