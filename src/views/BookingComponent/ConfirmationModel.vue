<template>
  <Modal :isOpen="openModalArchive" @closeModal="closeArchiveModal">
    <DialogPanel class="w-full max-w-3xl transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl transition-all">
      <DialogTitle as="div" class="text-lg font-medium leading-6 text-gray-900 mb-5 flex justify-between items-center">
      ...
      <div class="space-y-4">
        <p class="text-sm font-medium">Review all items before archiving this invoice</p>
        
        <!-- Items validation section -->
        <div class="max-h-96 overflow-y-auto border rounded-lg p-4">
          <div v-if="validationStatus.basic" class="flex items-center mb-3 text-sm">
          ...
          <!-- Items list -->
          <div v-for="(item, index) in formData.items" :key="index" class="mb-4 border-b pb-3">
            <!-- Special handling for items with defined product_type -->
            <div v-if="item.product_type !== undefined">
              <div class="flex justify-between items-center">
                <div class="font-medium">Item #{{ index + 1 }}: {{ getProductTypeName(item.product_type) }}</div>
                <div v-if="validationStatus.items[index]" class="flex items-center text-green-600">
              ...
              <div class="grid grid-cols-2 gap-2 mt-2 text-xs">
                ...
                <div class="flex">
                  ...
                  <ExclamationCircleIcon v-if="!item.service_date && needsServiceDate(item.product_type)" class="w-4 h-4 text-red-500 ml-1" />
                </div>
                <div class="flex" v-if="item.product_type != 4">
                  <span class="font-medium mr-1">Quantity:</span>
                  <span>{{ item.quantity || 'N/A' }}</span>
                  <ExclamationCircleIcon v-if="!item.quantity && needsQuantity(item.product_type)" class="w-4 h-4 text-red-500 ml-1" />
                </div>
                ...
                <div v-if="item.product_type == '1'" class="flex">
                  ...
                  <ExclamationCircleIcon v-if="!item.car_id" class="w-4 h-4 text-red-500 ml-1" />
                </div>
                ...
                <template v-if="item.product_type == '4'">
                  ...
                  <div class="flex">
                    ...
                    <span>{{ item.individual_pricing?.child?.quantity || '0' }}</span>
                  </div>
                  <div v-if="!item.quantity && !item.individual_pricing?.child?.quantity" class="col-span-2 text-red-500 text-xs">
                ...
                <template v-if="item.product_type == '6'">
                  <div class="flex">
                    ...
                    <ExclamationCircleIcon v-if="!item.checkin_date" class="w-4 h-4 text-red-500 ml-1" />
                  </div>
                  <div class="flex">
                    <span class="font-medium mr-1">Check-out:</span>
                    <span>{{ item.checkout_date || 'N/A' }}</span>
                    <ExclamationCircleIcon v-if="!item.checkout_date" class="w-4 h-4 text-red-500 ml-1" />
                  </div>
                </template>
              </div>
            
              <!-- Display validation errors if any -->
              <div v-if="!validationStatus.items[index]" class="mt-2 text-xs text-red-500">
                <div v-for="(error, errorIndex) in getItemValidationErrors(item)" :key="errorIndex">
                  <ExclamationCircleIcon class="w-4 h-4 inline mr-1" />
                  {{ error }}
                </div>
              </div>
            ...
            <div v-else class="flex justify-between items-center">
              <div class="font-medium">Item #{{ index + 1 }}: <span class="text-gray-500">Additional Item</span></div>
        ...
        <div class="border rounded-lg p-4 space-y-2">
          ...
          <div class="grid grid-cols-2 gap-y-2 text-sm">
            <div class="flex items-center">
              <div v-if="validationStatus.payment" class="bg-green-500 w-3 h-3 mr-2 inline-block rounded-full"></div>
              <div v-else class="bg-orange-400 w-3 h-3 mr-2 inline-block rounded-full"></div>
              <span>Payment ({{ formData.payment_method || 'Not specified' }})</span>
            </div>
            
            <div class="flex items-center">
              <div v-if="validationStatus.tax" class="bg-green-500 w-3 h-3 mr-2 inline-block rounded-full"></div>
              <div v-else class="bg-orange-400 w-3 h-3 mr-2 inline-block rounded-full"></div>
              <span>Tax Information</span>
            </div>
            
            <div class="flex items-center">
              <div v-if="validationStatus.discount" class="bg-green-500 w-3 h-3 mr-2 inline-block rounded-full"></div>
              <div v-else class="bg-orange-400 w-3 h-3 mr-2 inline-block rounded-full"></div>
              <span>Discount ({{ sub_total_discount || '0' }})</span>
            </div>
            
            <div class="flex items-center">
              <div v-if="validationStatus.total" class="bg-green-500 w-3 h-3 mr-2 inline-block rounded-full"></div>
              <div v-else class="bg-orange-400 w-3 h-3 mr-2 inline-block rounded-full"></div>
        ...
        <div class="pt-2">
          <div class="w-full bg-gray-200 rounded-full h-2.5 mb-4">
            <div class="bg-green-500 h-2.5 rounded-full" :style="{ width: validationProgress + '%' }"></div>
          </div>
          ...
          <div class="flex justify-between items-center gap-x-4">
            <button
              ...
              :class="isValidationComplete ? 'bg-[#ff613c] hover:bg-[#e04e2e]' : 'bg-gray-400'"
...
const validateBasicInfo = () => {
  ...
  validationStatus.value.basic = !!(props.formData && 
...
const validateItem = (item, index) => {
  ...
  switch (item.product_type) {
    ...
    case "4": // Entrance Ticket
      ...
      if ((!item.quantity && !item.individual_pricing?.child?.quantity) || 
          (Number(item.quantity) == 0 && Number(item.individual_pricing?.child?.quantity) == 0)) {
        isValid = false;
      }
      break;
...
const runValidation = () => {
  ...
  if (validationStatus.value.items.length !== props.formData.items.length) {
    validationStatus.value.items = Array(props.formData.items.length).fill(false);
  }
...
const isValidationComplete = computed(() => {
  return validationStatus.value.basic &&
...
watch(() => props.openModalArchive, (isOpen) => {
  if (isOpen) {
    runValidation();
  }
});
...
watch(() => props.formData?.items, (items) => {
