<template>
  <div class="flex justify-start items-center gap-x-4">
    ...
    <Modal :isOpen="showModal" @closeModal="showModal = false">
      <DialogPanel
        class="w-full max-w-7xl transform overflow-hidden rounded-lg bg-white text-left align-middle shadow-xl transition-all"
      >
        <DialogTitle
          as="div"
          class="text-sm text-white bg-[#FF613c] font-medium leading-6 flex justify-between items-center py-2 px-4"
        >
          <p>Check Allotment - All Rooms</p>
          <XCircleIcon
...
// New refs for all rooms data
const allRoomsData = ref([]);
const isLoadingRoomData = ref(false);

// Function to search all rooms
const searchAllRooms = async () => {
  ...
  try {
    // Loop through all rooms in the selected hotel
    const roomPromises = rooms.value.map(async (room) => {
      ...
      if (res.status === 1 && res.data) {
        // Calculate total cost and check if fully booked
        let totalCost = 0;
        ...
        for (const [date, rateData] of Object.entries(res.data.room_rates)) {
          ...
          // If any day has insufficient rooms, mark as fully booked
        ...
        return {
          ...
          isExpanded: false, // For dropdown toggle
        };
      }
      return null;
    });
