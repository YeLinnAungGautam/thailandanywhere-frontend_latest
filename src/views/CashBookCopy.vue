<template>
  <Layout :is_white="true" class="relative">
                  ...
                  >
                    <div class="flex flex-col gap-1">
                      <span
                        ...
                        class="bg-blue-100 text-blue-600 whitespace-nowrap px-2 py-1 rounded-full text-[10px]"
                      >
                        {{ item.cash_images.length }} detailed
                      </span>
                      <span
                        v-if="item?.cash_book_images?.length > 0"
                        class="bg-green-100 text-green-600 px-2 py-1 whitespace-nowrap rounded-full text-[10px]"
                      >
            ...
            <div class="col-span-1 p-4 rounded-lg shadow space-y-4">
              ...
              <div class="space-y-3">
                <div class="space-y-3">
                  ...
                  <!-- In your template where you display images -->
                  <div
                  ...
                  >
                    <div class="flex justify-between items-center">
                      ...
                      <div class="flex items-center gap-2">
                        <button
                          @click="openImageModal('cashbook', index)"
                    ...
                    <!-- Image Preview - Updated to show previews -->
                    <div class="flex items-center gap-3">
                      ...
                      <div class="text-[10px] text-gray-600 flex-1">
                        <p v-if="imageType === 'cash'">
                          <strong>Sender:</strong>
                          {{ image.sender || "Not set" }}
                        </p>
                        <p v-if="imageType === 'cash'">
                          <strong>Receiver:</strong>
                          {{ image.receiver || "Not set" }}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    No simple images added yet. Click "Add Simple Image" to
                    upload basic images.
                  </div>
                </div>
              ...
              <div class="space-y-3">
                ...
                >
                  <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center gap-2">
                      <button
                        @click="openImageModal('cash', index)"
                        class="text-blue-500 text-xs hover:text-blue-700"
                      >
                        Edit
                      </button>
                  ...
                  <div class="flex items-center gap-3">
                    <img
                      v-if="image.existing_image || image.preview"
                      :src="image.preview || image.existing_image"
                      alt="Image preview"
                      class="w-16 h-16 object-cover rounded border"
                    />
                    ...
                    <div class="text-[10px] text-gray-600 flex-1">
                      <p>
                        <strong>Sender:</strong> {{ image.sender || "Not set" }}
                      </p>
                      <p>
                        <strong>Receiver:</strong>
                        {{ image.receiver || "Not set" }}
    ...
    <ImageModal
      ...
      :isEditing="imageModal.isEditing"
      @close="closeImageModal"
      @save="saveImageFromModal"
    />
  </Layout>
</template>
...
import ImageModal from "../components/ModalLayout/ImageModal.vue"; // Import the new modal component
...
// In your parent component's editGetFormData method
const editGetFormData = (data) => {
  ...
  formData.value.images =
    data.cash_images?.map((img) => ({
      ...
      preview: img.image, // Add preview for existing images
      is_existing: true,
    })) || [];
...
const openImageModal = (type, index = -1) => {
  imageModal.value.type = type;
  imageModal.value.currentIndex = index;
  imageModal.value.isEditing = index >= 0;

  if (index >= 0) {
    // Editing existing image
    if (type === "cash") {
      imageModal.value.data = { ...formData.value.images[index] };
    } else {
      imageModal.value.data = { ...formData.value.cash_book_images[index] };
    }
  } else {
    // Adding new image
    if (type === "cash") {
      // Get current datetime for new cash images
      const now = new Date();
      const formattedNow = formatDateTimeForInput(now);

      imageModal.value.data = {
        id: null,
        image: null,
        sender: "",
        receiver: "",
...
const closeImageModal = () => {
  ...
  imageModal.value.data = {};
  imageModal.value.currentIndex = -1;
  imageModal.value.isEditing = false;
};

// In your parent component
const saveImageFromModal = (imageData) => {
  if (imageModal.value.type === "cash") {
    if (imageModal.value.isEditing) {
      // Update existing image
      formData.value.images[imageModal.value.currentIndex] = {
        ...imageData,
    ...
    if (imageModal.value.isEditing) {
      ...
      formData.value.cash_book_images[imageModal.value.currentIndex] = {
        ...imageData,
        is_existing: imageData.id ? true : false,
      };
    } else {
      // Add new cash book image
...
// Legacy image management functions (kept for compatibility)
