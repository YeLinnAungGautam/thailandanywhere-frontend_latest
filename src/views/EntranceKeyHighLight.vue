...
Attraction Name: ${props.highlightData?.name || "N/A"}
Description: ${props.highlightData?.description || "N/A"}
Full Description (English): ${props.highlightData?.full_description_en || "N/A"}
Location: ${props.highlightData?.location_map_title || "N/A"}
City: ${props.highlightData?.cities?.map((c) => c.name).join(", ") || "N/A"}
Categories: ${
      props.highlightData?.categories?.map((c) => c.name).join(", ") || "N/A"
    }
Ticket Price: ${props.highlightData?.lowest_variation_price || "N/A"} THB
Walk-in Price: ${props.highlightData?.lowest_walk_in_price || "N/A"} THB
`;

    const prompt = `You are a travel marketing expert creating key highlight titles and descriptions for a tourist attraction/entertainment venue in Thailand.

Based on this attraction information:
${highlightContext}

Generate exactly 8 key highlights in JSON format. Each highlight should emphasize the most attractive and unique features that would make visitors want to visit this attraction.

Requirements:
1. Create 8 unique highlights focusing on what makes this attraction special:
   - Unique experiences and main attractions
   - Visual spectacles or photo opportunities  
   - Cultural, educational, or entertainment value
   - Interactive activities and exhibits
   - Accessibility and convenience (location, parking, facilities)
   - Family-friendly features and age suitability
   - Special exhibits, zones, or themed areas
   - Comfort features (air-conditioning, seating, etc.)
   
2. Each title should be catchy and concise (3-6 words) in English language only
...
5. Set order from 0 to 7 (most impressive/important first)
6. All items should be active (is_active: true)
7. DO NOT include any image URLs or image-related fields
8. Focus on what makes this attraction special, unique, and worth visiting
9. Make descriptions exciting and appealing to international tourists

Example highlight types for attractions:
- "Immersive Art Experience" - Unique interactive installations and displays
- "Instagram-Worthy Displays" - Perfect photo opportunities throughout
- "Educational Journey" - Learn about art, culture, or technology
- "Family-Friendly Activities" - Great experiences for all ages
- "Climate Controlled Comfort" - Air-conditioned indoor venue
- "Convenient Central Location" - Easy to reach from main areas
- "Cutting-Edge Technology" - Modern media art and digital displays
- "Authentic Thai Culture" - Local contemporary art scene
- "Interactive Exhibits" - Hands-on experiences and participation
- "Multiple Themed Zones" - Various areas to explore

Return ONLY valid JSON in this exact format (no markdown, no additional text, no code blocks):
[
  {
    "title": "Immersive Digital Art",
    "description_mm": "နည်းပညာခေတ်မီ ဒစ်ဂျစ်တယ်အနုပညာပြခန်းများဖြင့် တစ်မျိုးတစ်ဖုံထူးခြားသော အတွေ့အကြုံများရရှိမည်။ အပြန်အလှန်တုံ့ပြန်နိုင်သော မီဒီယာအနုပညာများကို တိုက်ရိုက်ခံစားနိုင်ပြီး မိမိကိုယ်တိုင် ပါ၀င်ဆောင်ရွက်နိုင်ပါသည်။",
    "description_en": "Experience extraordinary moments with cutting-edge digital art exhibitions. Enjoy interactive media art installations that respond to your movements and create unique visual experiences you can participate in.",
    "order": 0,
    "is_active": true
  },
  {
    "title": "Perfect Photo Opportunities",
    "description_mm": "Instagram နှင့် Social Media အတွက် အကောင်းဆုံး ဓာတ်ပုံရိုက်ကူးနိုင်သော နေရာများစွာရှိပါသည်။ အရောင်အသွေးစုံသော အလင်းရောင်များ၊ ထူးခြားသောနောက်ခံများဖြင့် မှတ်မိဖွယ်ရာ ဓာတ်ပုံများရိုက်ကူးနိုင်ပါသည်။",
    "description_en": "Numerous Instagram-worthy spots throughout the venue perfect for social media. Capture memorable photos with colorful lighting displays, unique artistic backdrops, and stunning visual effects.",
    "order": 1,
    "is_active": true
  },
  {
    "title": "Air-Conditioned Comfort",
    "description_mm": "အေးအေးချမ်းချမ်း လေအေးပေးစက်ရှိသော အဆောက်အအုံအတွင်းတွင် သက်သောင့်သက်သာ ခံစားနိုင်ပါသည်။ ထိုင်းနိုင်ငံ၏ ပူပြင်းသောရာသီဥတုမှ ခေတ္တရှောင်ကာ အနုပညာလက်ရာများကို အေးချမ်းစွာ ကြည့်ရှုခံစားနိုင်ပါသည်။",
    "description_en": "Enjoy the attraction in complete comfort with full air-conditioning throughout the venue. A perfect escape from Thailand's tropical heat while exploring amazing art installations in cool comfort.",
    "order": 2,
    "is_active": true
  },
  {
    "title": "Family-Friendly Experience",
    "description_mm": "မိသားစုတစ်စုလုံးအတွက် သင့်လျော်သော အပျော်အပါးဖြစ်ပါသည်။ ကလေးငယ်များမှ လူကြီးများအထိ အားလုံး ပျော်ရွှင်စွာ ခံစားနိုင်မည့် အတွေ့အကြုံများ ပါ၀င်ပါသည်။",
    "description_en": "Perfect entertainment for the whole family with experiences suitable for all ages. Children and adults alike will enjoy the interactive displays and engaging exhibitions.",
    "order": 3,
    "is_active": true
  }
]

Focus on creating compelling, exciting highlights that would attract visitors to this Thailand attraction. Make descriptions vivid and appealing to tourists!`;
    ...
    if (result.isConfirmed) {
      try {
        for (let i = 0; i < highlights.value.length; i++) {
          await store.deleteAction(highlights.value[i].id);
...
onMounted(() => {
  console.log("highlightData", props.highlightData);
  if (
