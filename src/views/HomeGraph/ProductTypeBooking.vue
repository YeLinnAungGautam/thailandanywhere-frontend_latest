...
const dateLabels = reactive({ items: [] });
...
const chartOptions = {
  ...
  onClick: (event, elements) => {
    if (elements.length > 0) {
      ...
      const date = dateLabels.items[index];
...
const getProductTypeData = async (dateRange) => {
  ...
  try {
    ...
    // Clear existing data
    dateLabels.items.splice(0);
    chartData.datasets = [];
    totalBookings.value = 0;
    productTypeTotals.value = new Map();
    // Clear booking items when date range changes
    bookingItems.value = [];
    ...
    result.forEach((item) => {
      dateLabels.items.push(item.date);
    });
    ...
    result.forEach((dateItem) => {
      ...
      dateItem.product_types.forEach((pt) => {
        ...
        productTypesInDate.set(productType, pt.booking_item_count_sale);
        totalBookings.value += pt.booking_item_count_sale;

        // Update product type totals
        const currentTotal = productTypeTotals.value.get(productType) || 0;
        productTypeTotals.value.set(
          productType,
          currentTotal + pt.booking_item_count_sale
        );
      });
...
// Fetch booking items for specific product type and date
const fetchBookingItems = async (productType, date) => {
  ...
  try {
    ...
    const response = await homeStore.getProductTypeBookingItem(params);
