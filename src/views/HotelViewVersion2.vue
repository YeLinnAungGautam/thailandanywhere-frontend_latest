			...
			<div class="hidden p-6 mb-5 rounded-lg shadow-sm bg-white/60 md:col-span-3 md:block">
				<div class="flex gap-4 mb-5">
					<div class="bg-white rounded-xl border border-gray-200 shadow-sm px-5 py-5 w-48">
						<div class="flex items-center gap-2 mb-1">
							<span class="text-2xl font-medium text-black-800">{{ totalHotelsCount }}</span>
							<span class="text-sm text-green-600 font-semibold" v-if="totalHotelsGrowth > 0">
								▲ {{ totalHotelsGrowth }}%
							</span>
							<span class="text-sm text-red-600 font-semibold" v-else-if="totalHotelsGrowth < 0">
					...
					<div class="bg-white rounded-xl border border-gray-200 shadow-sm px-5 py-5 w-48">
						<div class="flex items-center gap-2 mb-1">
							<span v-if="displayedHotelsCount > 0" class="text-2xl font-medium text-black-800">{{
				...
				<div class="flex gap-6">
                    ...
                    >
                      ...
                      Custom Price Range
							...
							<div class="mb-5 border-b pb-4">
								...
								<div v-if="showArea" class="space-y-1 max-h-40 overflow-y-auto pr-1">
									<label
										class="flex items-center gap-2 text-xs cursor-pointer hover:bg-gray-50 rounded-md p-2"
										:class="selectedArea === '' ? 'text-[#FF5B00]' : 'text-black'"
										@click="selectArea('')"
									>
										<input type="radio" :checked="selectedArea === ''" class="accent-[#FF5B00]" />
										All Areas
									</label>
									...
									<label
										...
										:key="area"
										class="flex items-center gap-2 text-xs cursor-pointer hover:bg-gray-50 rounded-md p-2"
										:class="selectedArea === area ? 'text-[#FF5B00]' : 'text-black'"
										@click="selectArea(area)"
									>
										<input type="radio" :checked="selectedArea === area" class="accent-[#FF5B00]" />
										{{ area }}
									</label>
								</div>
							</div>
							...
							<div>
								...
								<div class="flex scroller-none gap-2 max-h-40 overflow-x-auto pr-1 scrollbar-hide">
									<button
									...
									<div v-if="filteredAmenities.length === 0" class="text-xs text-gray-500 py-2 px-3">
										No amenities found
									</div>
								</div>
							</div>
					...
					<div class="flex-1">
						<div v-if="!loading" class="grid grid-cols-1 md:grid-cols-2 gap-6">
							...
							>
								<div class="relative h-40 bg-gray-100 overflow-hidden">
									<img :src="getHotelImage(hotel)" :alt="hotel.name" class="w-full h-full object-cover" />
								...
								<div class="p-4">
									<div class="flex justify-between items-center mb-3 border-b pb-3">
										<div>
											<h4 class="text-lg font-semibold text-[#FF5B00] leading-tight">
												{{ hotel.name }}
											</h4>
									...
									<div class="space-y-1">
										...
										>
											...
											<span class="text-right font-medium text-black-700"> ฿ {{ room.room_price }} </span>
										</div>
									</div>
								</div>
							</div>
						...
						<div v-if="!loading && displayedHotels.length === 0" class="text-center py-20">
							...
							<h4 class="text-lg font-medium text-gray-700 mb-2">No hotels found</h4>
						...
						<div v-if="!loading" class="mt-6">
							...
							<Pagination v-if="!selectedFilter && hotels?.data?.length > 0" :data="hotels" @change-page="changePage" />
						</div>
					</div>
				</div>
				...
				<Modal :isOpen="importModal" @closeModal="importModal = false">
					...
					>
						<DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900 mb-5"> Import Hotels </DialogTitle>
						<form @submit.prevent="importActionHandler" class="space-y-4">
							<div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
								...
								<p class="text-sm text-gray-600 mb-3">Upload CSV file with hotel data</p>
								<input
									type="file"
						...
						<div class="mt-6 space-y-3 border border-gray-200 p-4 rounded-lg">
							...
							<ul class="text-xs text-gray-600 space-y-1">
								...
								<li>• Include all required columns (name, city, rating, etc.)</li>
...
const priceCategories = [
	...
	{ id: "luxury", name: "Luxury Price (2501 and above)", min: 2501, max: 50000 },
];
...
const getHotelPrice = (hotel) => {
	if (!hotel.rooms || !Array.isArray(hotel.rooms) || hotel.rooms.length === 0) return 0;
	const prices = hotel.rooms.map((room) => parseInt(room.room_price) || 0).filter((price) => price > 0);
	...
	if (selectedFilter.value) {
		filtered = filtered.filter((hotel) => {
			switch (selectedFilter.value) {
				...
				case "images":
					return !hotel.images || hotel.images === "null" || !Array.isArray(hotel.images) || hotel.images.length === 0;
				case "facilities":
					return (
						!hotel.facilities ||
				...
				case "rating":
					return !hotel.rating || hotel.rating === "null" || hotel.rating.trim() === "";
				case "nearby_places":
					return !hotel.nearby_places || !Array.isArray(hotel.nearby_places) || hotel.nearby_places.length === 0;
				case "bank_name":
					return !hotel.bank_name || hotel.bank_name === "null" || hotel.bank_name.trim() === "";
				case "place":
					return !hotel.place || hotel.place === "null" || hotel.place.trim() === "";
...
const fetchHotels = async () => {
	...
	if (selectedPriceCategory.value !== "all") {
		...
		if (category) {
			...
			// } else {
			//   params.price_range = `${category.min}-${category.max}`;
			// }
		}
	}
...
const fetchTotalHotelsCount = async () => {
	try {
		...
		try {
			const retryResponse = await hotelStore.getListAction({ limit: 1000, page: 1 });
...
onMounted(async () => {
	...
	await Promise.all([cityStore.getListHotelCityAction({ limit: 100 }), facilityStore.getListAction()]);
