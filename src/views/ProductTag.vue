<template>
  ...
  >
    ...
    <div class="bg-white/60 p-6 rounded-lg shadow-sm mb-5">
      ...
      <!-- pagination -->
      <Pagination v-if="!loading" :data="tags" @change-page="changePage" />
    </div>
    <!-- modal -->
    <Modal :isOpen="carModalOpen" @closeModal="carModalOpen = false">
      ...
      >
        ...
        >
          {{ formData.id ? "Edit Product Tags" : "Add Product Tags" }}
        </DialogTitle>
...
const search = ref("");
const { tags, loading } = storeToRefs(productStore);

const changePage = async (url) => {
  await productStore.getChangeTagPage(url);
};
...
const addNewHandler = async () => {
  ...
  try {
    const response = await productStore.addNewTagAction(frmData);
    formData.value = {
      name: "",
    };
    errors.value = null;
    carModalOpen.value = false;
    await productStore.getListTagAction();
...
const updateHandler = async () => {
  ...
  try {
    const response = await productStore.updateTagAction(
      frmData,
      formData.value.id
    );
    formData.value = {
...
const onDeleteHandler = async (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        const response = await productStore.deleteTagAction(id);
        toast.success(response.message);
      } catch (error) {
        if (error.response.data.errors) {
          errors.value = error.response.data.errors;
