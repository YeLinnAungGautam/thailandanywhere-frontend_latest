    ...
    try {
      ...
      if (formData.value.images.length > 0) {
        try {
          ...
          const uploadRes =
            await reservationStore.ReservationBookingRequestAction(
              reservation_ids.value[a],
              secData
            );
...
const removeImageUpdateImage = async (id, imageID) => {
  try {
    const res = reservationStore.ReservationBookingRequestDeleteAction(
      id,
      imageID
    );
...
onMounted(() => {
  ...
  if (props?.detail) {
    ...
    formData.value.editImagesPreview =
      props?.detail?.booking?.items[0]?.booking_requests ?? [];
