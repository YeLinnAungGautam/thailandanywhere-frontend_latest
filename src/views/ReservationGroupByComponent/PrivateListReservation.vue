            ...
            <div>
              <p class="text-[10px] space-x-1">
                <span class="font-medium"
                  >Book: {{ getFormatDate(i?.booking_date) }} / Due:
                  {{ getFormatDate(i?.balance_due_date) }}</span
                >
              </p>
            </div>
            <div class="">
...
<script setup>
  ...
  const customerPassportStatus = (i) => {
    ...
    // Check the status of each item
    for (let a = 0; a < items.length; a++) {
      const item_status = items[a].reservation_car_info != null && items[a].reservation_car_info.driver_id ? true : false;
