      ...
      <div class="pl-5 pt-2 gap-y-4 gap-x-3">
          ...
          <div>
            <p class="text-[10px] space-x-1">
              <span class="font-medium"
                >Book: {{ getFormatDate(i?.booking_date) }}</span
              >
            </p>
          </div>
          <div class="">
        ...
        <div class="pt-1 flex justify-start items-center gap-x-3">
          ...
          >
            ...
            <p class="text-[10px] whitespace-nowrap text-green-600">Assigned</p>
          </div>
          <div
            class="flex justify-start space-x-1 items-center"
        ...
        <div v-if="showList" class="">
          ...
          >
            <div v-for="item in i.items" :key="item.id" class="">
              <div>
                <div
                  class="text-[12px] font-medium space-x-2 text-gray-900 flex justify-between items-center overflow-x-scroll no-sidebar-container pt-2"
                  :class="index == 0 ? '' : ' border-t border-gray-200'"
                >
                  <div>
                    <p
                      v-if="item.product_type == 'App\\Models\\PrivateVanTour'"
                      class="whitespace-nowrap font-medium text-[10px]"
                    >
                      {{ item.car?.name }}
                    </p>
                    <p
                      class="whitespace-nowrap text-[12px] w-[150px] line-clamp-1"
                    >
                      {{ item.product?.name }}
                    </p>
                  </div>
...
const customerPassportStatus = (i) => {
  ...
  for (let a = 0; a < items.length; a++) {
    const item_status =
      items[a].reservation_car_info != null &&
      items[a].reservation_car_info.driver_id
        ? true
        : false;
