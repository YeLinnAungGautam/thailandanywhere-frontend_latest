        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            ...
            <span class="text-blue-800 font-medium"
              >Editing Tax Receipt #{{ route.query.id }}</span
            >
          </div>
        ...
        <div class="space-y-3 pt-2">
          ...
          <div class="space-y-1.5">
            ...
            >
              ...
              <option value="App\Models\EntranceTicket">Entrance Ticket</option>
            </select>
          </div>
          ...
          <!-- Company Legal Name (Auto-filled and disabled) -->
          <div class="space-y-1.5">
              ...
              >
                <div v-if="!imagePreview" class="text-center">
                  ...
                  <p class="text-gray-400 text-sm">
                    {{
                      isEditMode ? "Change tax image" : "Upload tax image here"
                    }}
                  </p>
                  <p class="text-gray-300 text-xs mt-1">Click or drag & drop</p>
                </div>
...
const loadTaxReceiptDetail = async (id) => {
  try {
    ...
    if (data.receipt_image) {
      // Assuming the image path needs to be converted to full URL
...
const onProductTypeChange = () => {
  if (!isEditMode.value) {
    // Reset product selection when type changes (only for create mode)
...
const goToProductDetail = () => {
  if (formData.product_id) {
    console.log("Navigate to product detail:", formData.product_id);
...
const cancelEdit = () => {
  // Navigate back or reset to original data
  router.push({ query: {} }); // Remove query params
  createNewTax();
};
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      ...
      // Optionally redirect back to list or stay on edit
      // router.push('/tax-receipts');
    } else {
...
const deleteTaxReceipt = async () => {
  try {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: `Do you want to delete Tax Receipt #${route.query.id}? This action cannot be undone!`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      ...
      // Call the delete API
      await taxReceiptStore.deleteAction(route.query.id);
