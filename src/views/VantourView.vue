...
const addNewSupplierCost = () => {
  editData.value.supplier_cost.push(formSupplierCost.value);
...
const removeFromSupplierCost = (index) => {
  editData.value.supplier_cost.splice(index, 1);
};
...
const onSubmitHandler = async () => {
  ...
  for (let x = 0; x < editData.value.supplier_cost.length; x++) {
    const sc = editData.value.supplier_cost[x];
    frmData.append(`supplier_cost[${x}][supplier_id]`, sc.supplier_id);
    // frmData.append(`supplier_cost[${x}][van_price]`, sc.van_price);
    // frmData.append(`supplier_cost[${x}][suv_price]`, sc.suv_price);
    // frmData.append(`supplier_cost[${x}][saloon_price]`, sc.saloon_price);
...
const getDetail = async () => {
  try {
    ...
    formData.value.is_show = response.result.is_show ?? true;
    editData.value.supplier_cost = response.result.supplier_cost ?? [];
        ...
        <div class="bg-white/60 col-span-2 p-6 rounded-lg shadow-sm mb-5">
          <div class="space-y-4">
            ...
            <div class="col-span-2">
              ...
              <div
                v-for="(sc, index) in editData.supplier_cost"
                :key="index"
